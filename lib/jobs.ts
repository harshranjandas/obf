import { Job } from '@/types/type';

export const jobs: Job[] = [
  {
    id: 'junior-content-writer',
    title: 'Junior Content Writer',
    subTitle: '2-3 years experience',
    location: 'Delhi (IN)',
    locationType: 'On-Site',
    type: 'Full-Time',
    logo: '/images/job-logo.png',
    theRole:
      'Support long-form editorial content across podcasts, interviews, and events at One Big Future.',
    whatYouDo: [
      'Draft long-form articles, interview features, and podcast-to-text pieces',
      'Support background research, briefing notes, and editorial prep',
      'Edit transcripts and assist with revisions, fact-checking, and accuracy checks',
      'Help structure narratives under senior editorial guidance',
      'Collaborate with campaigns to adapt content across formats',
    ],
    whatWereLookingFor: [
      'Strong writing skills with a knack for storytelling',
      'Experience in podcast transcription and editing',
      'Attention to detail and accuracy in fact-checking',
      'Ability to collaborate with diverse teams',
      'Familiarity with podcasting and event journalism',
    ],
    preferredExperience: [
      '2–3 years in long-form writing, research, or editorial roles',
      'Exposure to reportage, features, policy writing, or think tanks',
      'Podcast-to-text or interview-based writing is a plus',
      'SEO-only or purely marketing-led content is not a fit',
    ],
  },
  {
    id: 'senior-content-writer',
    title: 'Senior Content Writer',
    subTitle: '5-7 years experience',
    location: 'Delhi (IN)',
    locationType: 'On-Site',
    type: 'Full-Time',
    logo: '/images/job-logo.png',
    theRole:
      'Own end-to-end long-form editorial content across podcasts, interviews, and events at One Big Future.',
    whatYouDo: [
      'Lead creation of long-form articles, interview features, and podcast-to-text pieces',
      'Translate complex conversations, research, and ideas into clear, structured narratives',
      'Conduct deep background research and prepare editorial briefs for interviews, podcasts, and events',
      'Shape narrative framing, arguments, and thematic flow, ensuring accuracy and editorial rigor',
      'Review, edit, and provide feedback on junior writers’ drafts',
      'Collaborate with campaigns to adapt content across formats',
    ],
    whatWereLookingFor: [
      'Excellent long-form writing and editing skills with strong editorial judgment',
      'Proven ability to engage deeply with entrepreneurship, policy, technology, economy, and culture',
      'Strong research capability and narrative clarity',
      'Experience working with founders, CXOs, or senior leaders',
      'India-focused perspective with global awareness',
    ],
    preferredExperience: [
      '5–7 years in long-form writing, research, or editorial roles',
      'Background in reputed media, policy writing, academia, or think tanks',
      'Strong portfolio of essays, features, interviews, or thought leadership',
      'Experience with podcast-to-text or interview-led formats',
      'SEO-only or purely marketing-led content is not a fit',
    ],
  },
];
