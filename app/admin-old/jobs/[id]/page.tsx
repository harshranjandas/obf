  ...
  const fetchJob = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          title: job.title || '',
          company: job.subTitle || '',
          type: job.type || '',
          companyWebsite: job.logo || '',
          location: job.location || '',
          locationType: job.locationType || '',
          notificationEmails: '',
          jobDescription: job.theRole || '',
          displayOrder: job.order?.toString() || '',
          theRoleTitle: 'The Role',
          theRoleDescription: job.theRole || '',
          whatYouDoTitle: "What You'll Do",
          whatYouDoDescription: Array.isArray(job.whatYouDo) ? job.whatYouDo.join('\n') : '',
          whatWereLookingForTitle: "What We're Looking For",
          whatWereLookingForDescription: Array.isArray(job.whatWereLookingFor) ? job.whatWereLookingFor.join('\n') : '',
          lifeAtCompanyTitle: '',
          lifeAtCompanyDescription: '',
          showUsYourWorkTitle: 'Show Us Your Work',
          showUsYourWorkDescription: '',
          status: job.published ? 'published' : 'unpublished',
        })
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const whatWereLookingFor = formData.whatWereLookingForDescription
        ...
        : []
      const preferredExperience: string[] = []
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="mb-4 text-sm text-gray-600">
        ...
        <span>{formData.title || 'Edit Job'}</span>
      </div>
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          ...
          <p className="text-gray-600">Editing Job</p>
        </div>
        <button
